...
const lines = items.map(function(i) { return "  - " + i.tech.firstName + " " + i.tech.lastName + " - " + i.label + " (expired " + fmt(i.exp) + ", " + i.daysOver + " days ago)"; }).join("\n");
return {
...
if (!GMAIL_USER || !GMAIL_PASSWORD) { console.error("ERROR: Missing secrets."); process.exit(1); }
...
const exp = tech[cr.key]; if (!exp) continue;
const days = daysUntil(exp);
const cc = [SUPERVISOR_EMAIL, MANAGER_EMAIL].filter(function(e) { return e !== tech.email; }).join(", ");
if (days <= 0) {
const daysOver = Math.abs(days);
expired.push({ tech: tech, label: cr.label, exp: exp, daysOver: daysOver });
const em = expiredEmail(tech, cr.label, exp, daysOver);
try { await transport.sendMail({ from: from, to: tech.email, cc: cc || undefined, subject: em.subject, text: em.text }); console.log("SENT expired: " + tech.firstName + " " + tech.lastName); sent++; }
catch(e) { console.error("FAILED: " + tech.firstName + " " + tech.lastName + " | " + e.message); failed++; }
} else if (REMINDER_DAYS.includes(days)) {
const em = upcomingEmail(tech, cr.label, exp, days);
try { await transport.sendMail({ from: from, to: tech.email, cc: cc || undefined, subject: em.subject, text: em.text }); console.log("SENT " + days + "-day: " + tech.firstName + " " + tech.lastName); sent++; }
catch(e) { console.error("FAILED: " + tech.firstName + " " + tech.lastName + " | " + e.message); failed++; }
}
}
}
if (expired.length > 0) {
const em = managerSummaryEmail(expired);
try { await transport.sendMail({ from: from, to: [SUPERVISOR_EMAIL, MANAGER_EMAIL].join(", "), subject: em.subject, text: em.text }); console.log("Manager summary sent."); }
catch(e) { console.error("Manager summary failed:", e.message); }
}
